...
function BriefReport() {
  ...
  if (!response) {
    return <h1>Loading</h1>; // Ensure to return the loading state
  }
            ...
            <div className="mainReportContainer">
              <div className="secondContainer">
                ...
                <div className="thirdRow">
                  ...
                  <div className="description">{response.desc}</div>
                  <div className="status">
                    ...
                    <div className="responseStatus">
                      ...
                      ) : response.status === 'Rectified' ? (
                        <p className='approveStatus'>Rectified</p>
                      ) : response.status === 'Rejected' ? (
                        <p className='rejectStatus'>Rejected</p>
                      ) : (response.status === 'Flagged' && user.role==='student') ? (
                        <p className='rejectStatus'>Flagged</p>
                            ) : response.status === 'pending' && user.role === 'student' ? (
                      <p className='pendingStyle'>{response.status}</p>
                    ) :
